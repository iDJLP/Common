...
- (void)initialization
{
    ...
    _titleLabelTextColor = [GColorUtil C6];
    ...
    _currentPageDotColor = [GColorUtil C6];
...
- (void)setupMainView
{
    ...
    UICollectionViewFlowLayout *flowLayout = [[RollCollectionViewLayout alloc] init];
...
- (void)setImagePathsGroup:(NSArray *)imagePathsGroup
{
    ...
    if (imagePathsGroup.count > 1) { // 由于 !=1 包含count == 0等情况
        ...
        WEAK_SELF;
        [NTimeUtil run:^{
            weakSelf.mainView.scrollEnabled = NO;
...
- (void)scrollToIndex:(int)targetIndex
{
    if (targetIndex >= _totalItemsCount) {
        if (self.infiniteLoop) {
            ...
            [self scrollToItem:targetIndex animated:NO];
        }
        return;
    }
    [self scrollToItem:targetIndex animated:YES];
}

- (void)scrollToItem:(int)targetIndex animated:(BOOL)animated {
    NSIndexPath *selectedIndexPath = [NSIndexPath indexPathForItem:targetIndex inSection:0];
    
//    CGPoint proposedOffset = CGPointMake(0, 0);
//    proposedOffset.x = selectedIndexPath.item * (_flowLayout.itemSize.width + _flowLayout.minimumLineSpacing);
//    
//    CGPoint contentOffset = CGPointMake(proposedOffset.x, 0);
//    [_mainView setContentOffset:contentOffset animated:animated];
    [_mainView scrollToItemAtIndexPath:selectedIndexPath atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:animated];
...
- (int)currentIndex
{
    ...
    if (_flowLayout.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        index = (_mainView.contentOffset.x + (_flowLayout.itemSize.width + _flowLayout.minimumLineSpacing) * 0.5) / (_flowLayout.itemSize.width + _flowLayout.minimumLineSpacing);
    } else {
...
- (void)adjustWhenControllerViewWillAppera
{
    ...
    if (targetIndex < _totalItemsCount) {
        [self scrollToItem:(int)targetIndex animated:NO];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:[GColorUtil imageNamed:@"home_banner_default"] options:SDWebImageCacheMemoryOnly];
        } else {
            UIImage *image = [GColorUtil imageNamed:imagePath];
            if (!image) {
