...
@property (nonatomic,strong)SingleInfoView *profitView;
@property (nonatomic,strong)SingleInfoView *marignView;
@property (nonatomic,strong)SingleInfoView *openPriceView;
@property (nonatomic,strong)SingleInfoView *closePriceView;
...
+ (CGFloat)heightOfCell:(NSMutableDictionary *)dict{
    ...
    CGFloat height = [GUIUtil fit:45]+[GUIUtil fit:10]+singleH*2+[GUIUtil fit:5];
...
- (void)setupUI{
    ...
    [self.contentView addSubview:self.profitView];
    [self.contentView addSubview:self.marignView];
    [self.contentView addSubview:self.openPriceView];
    [self.contentView addSubview:self.closePriceView];
...
- (void)autoLayout{
    ...
    [self.openPriceView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo([GUIUtil fit:25]);
        make.width.mas_equalTo([GUIUtil fit:155]);
        make.top.equalTo(self.profitView.mas_bottom).mas_offset([GUIUtil fit:5]);
    }];
    [self.closePriceView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo([GUIUtil fit:195]);
        make.width.mas_equalTo([GUIUtil fit:155]);
        make.top.equalTo(self.profitView.mas_bottom).mas_offset([GUIUtil fit:5]);
    }];
    [self.tpPriceView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo([GUIUtil fit:25]);
        make.width.mas_equalTo([GUIUtil fit:155]);
        make.top.equalTo(self.openPriceView.mas_bottom).mas_offset([GUIUtil fit:5]);
    }];
...
- (void)configCell:(NSMutableDictionary *)dict{
    ...
    _typeLabel.text = [NDataUtil stringWith:dict[@"closetype"]];
    NSString *profit = [NDataUtil stringWith:dict[@"profit"]];
    UIColor *color = [GColorUtil colorWithProfitString:profit];
    NSString *profitnum = [NDataUtil stringWith:dict[@"profitnum"]];
    NSString *profitp = [GUIUtil equalValue:profitnum with:@"1"]?CFDLocalizedString(@"点"):CFDLocalizedString(@"点_复数");
    profit = [NSString stringWithFormat:@"%@(%@%@)",[NDataUtil stringWith:dict[@"profit"]],profitnum,profitp];
    [_profitView configOfInfo:@{@"key":CFDLocalizedString(@"盈亏"),@"value":profit,@"color":color}];
    ...
    [_openPriceView configOfInfo:@{@"key":CFDLocalizedString(@"开仓价"),@"value":[NDataUtil stringWith:dict[@"buyprice"]]}];
    [_closePriceView configOfInfo:@{@"key":CFDLocalizedString(@"平仓价"),@"value":[NDataUtil stringWith:dict[@"sellprice"]]}];
    ...
    [_dateView configOfInfo:@{@"key":CFDLocalizedString(@"开仓时间"),@"value":[NDataUtil stringWith:dict[@"opentime"]]}];
    [_noView configOfInfo:@{@"key":CFDLocalizedString(@"平仓时间"),@"value":[NDataUtil stringWith:dict[@"closetime"]]}];
    
    
    if ([NDataUtil boolWithDic:dict key:@"isUnFold" isEqual:@"1"]) {
        [_masLineTop activate];
        _arrowImg.image = [GColorUtil imageNamed:@"trade_icon_under_up"];
        _tpPriceView.hidden = _slPriceView.hidden = _tpPointView.hidden = _slPointView.hidden = _tpPrecentView.hidden = _slPrecentView.hidden =    _feeView.hidden = _pryView.hidden =
        _dateView.hidden = _noView.hidden = NO;
    }else{
        [_masLineTop deactivate];
        _arrowImg.image = [GColorUtil imageNamed:@"trade_icon_under_down"];
        _tpPriceView.hidden = _slPriceView.hidden = _tpPointView.hidden = _slPointView.hidden = _tpPrecentView.hidden = _slPrecentView.hidden = _feeView.hidden = _pryView.hidden =
        _dateView.hidden = _noView.hidden = YES;
    }
}
...
- (UIView *)bgview{
    if (!_bgview) {
        ...
        _bgview.layer.borderColor = [GColorUtil C19].CGColor;
...
- (SingleInfoView *)profitView{
    if(!_profitView){
        _profitView = [[SingleInfoView alloc] init];
        _profitView.alignment = SingleInfoAlignmentSide;
        _profitView.needChangedStyle= YES;
        [_profitView configOfInfo:@{@"key":CFDLocalizedString(@"盈亏"),@"value":@"--"}];
    }
    return _profitView;
}
...
- (SingleInfoView *)openPriceView{
    if(!_openPriceView){
        _openPriceView = [[SingleInfoView alloc] init];
        _openPriceView.alignment = SingleInfoAlignmentSide;
        _openPriceView.needChangedStyle= YES;
        [_openPriceView configOfInfo:@{@"key":CFDLocalizedString(@"开仓"),@"value":@"--"}];
    }
    return _openPriceView;
}
- (SingleInfoView *)closePriceView{
    if(!_closePriceView){
        _closePriceView = [[SingleInfoView alloc] init];
        _closePriceView.alignment = SingleInfoAlignmentSide;
        _closePriceView.needChangedStyle= YES;
        [_closePriceView configOfInfo:@{@"key":CFDLocalizedString(@"最新平仓"),@"value":@"--"}];
    }
    return _closePriceView;
}
...
- (SingleInfoView *)noView{
    if(!_noView){
        ...
        [_noView configOfInfo:@{@"key":CFDLocalizedString(@"平仓时间"),@"value":@"--"}];
    }
    return _noView;
}
