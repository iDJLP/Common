...
@property (nonatomic,strong)CalculatorRow *row4;
...
@implementation Calculator1View
...
- (void)setupUI{
    ...
    [self.topView addSubview:self.row4];
...
- (void)autoLayout{
    ...
    [self.topView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo([GUIUtil fitWidth:375 height:200]);
    }];
    ...
    [self.calculatorBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.bottomView.mas_bottom).mas_offset([GUIUtil fit:20]);
        make.size.mas_equalTo([GUIUtil fitWidth:165 height:43]);
        CGFloat height = [GUIUtil fit:50]*8+[GUIUtil fit:60]+[GUIUtil fit:7]*2+[GUIUtil fit:110]+TOP_BAR_HEIGHT+IPHONE_X_BOTTOM_HEIGHT; make.bottom.mas_equalTo(-SCREEN_HEIGHT+height-[GUIUtil fit:20]);
    }];
    ...
    [self.row4 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.row3.mas_bottom);
        make.left.mas_equalTo([GUIUtil fit:0]);
        make.size.mas_equalTo([GUIUtil fitWidth:375 height:50]);
    }];
...
- (void)configView:(NSDictionary *)dict{
    ...
    _fieldRow2.minFloat = minfloat;
    _fieldRow3.minFloat = @"1";
...
- (void)calculatorAction{
    //校验计算的条件
    {
        if (_fieldRow1.text.length<=0||_fieldRow1.text.floatValue==0) {
            [HUDUtil showInfo:CFDLocalizedString(@"请输入开仓价格")];
            return;
        }
        if (_fieldRow2.text.length<=0) {
            [HUDUtil showInfo:CFDLocalizedString(@"请输入平仓价格")];
            return;
        }
        if (_fieldRow3.text.length<=0) {
            [HUDUtil showInfo:CFDLocalizedString(@"请输入开仓数量")];
            return;
        }
    }
    //计算
...
- (void)calculatorHander{
    ...
    NSString *close = _fieldRow2.text;
    NSString *price = [GUIUtil decimalSubtract:close num:open];
    NSString *amount = _fieldRow3.text;
    ...
    NSString *profit = [GUIUtil decimalMultiply:value num:price];
    if (_buyBtn.selected==NO) {
        profit = [GUIUtil decimalMultiply:profit num:@"-1"];
        if (profit.floatValue>0) {
            profit = [NSString stringWithFormat:@"+%@",profit];
        }
    }
    profit = [GUIUtil notRoundingString:profit afterPoint:4];
    NSString *profitRate = [GUIUtil decimalPercentDivide:profit num:margin];
    if ([profitRate hasSuffix:@"%"]) {
        profitRate = [profitRate substringToIndex:profitRate.length];
        if (profitRate.floatValue>0) {
            profitRate = [NSString stringWithFormat:@"+%@",profitRate];
        }
    }
    NSDictionary *marginDic = @{@"title":CFDLocalizedString(@"开仓保证金（USDT）"),@"text":margin};
    NSDictionary *profitDic = @{@"title":CFDLocalizedString(@"收益（USDT）"),@"text":profit};
    NSDictionary *profitRateDic = @{@"title":CFDLocalizedString(@"收益率（%）"),@"text":profitRate};
    [_row2 configView:marginDic];
    [_row3 configView:profitDic];
    [_row4 configView:profitRateDic];
    _enableEdit = NO;
}

- (void)resetAction{
    NSDictionary *marginDic = @{@"title":CFDLocalizedString(@"开仓保证金（USDT）"),@"text":@"",@"placehoder":@"--"};
    NSDictionary *profitDic = @{@"title":CFDLocalizedString(@"收益（USDT）"),@"text":@"",@"placehoder":@"--"};
    NSDictionary *profitRateDic = @{@"title":CFDLocalizedString(@"收益率（%）"),@"text":@"",@"placehoder":@"--"};
    NSDictionary *openDic = @{@"title":CFDLocalizedString(@"开仓价格（USDT）"),@"text":@"",@"placehoder":CFDLocalizedString(@"请输入开仓价格")};
    NSDictionary *closeDic = @{@"title":CFDLocalizedString(@"平仓价格（USDT）"),@"text":@"",@"placehoder":CFDLocalizedString(@"请输入平仓价格")};
    NSDictionary *amountDic = @{@"title":CFDLocalizedString(@"开仓数量（手）"),@"text":@"",@"placehoder":CFDLocalizedString(@"请输入开仓数量")};
    [_row2 configView:marginDic];
    [_row3 configView:profitDic];
    [_row4 configView:profitRateDic];
    [_fieldRow1 configView:openDic];
    [_fieldRow2 configView:closeDic];
    [_fieldRow3 configView:amountDic];
...
- (CalculatorRow *)row4{
    if (!_row4) {
        _row4 = [[CalculatorRow alloc] init];
        _row4.isRightLabel = YES;
    }
    return _row4;
}
...
- (CalculatorRow *)fieldRow2{
    if (!_fieldRow2) {
        ...
        _fieldRow2.textField.keyboardType = UIKeyboardTypeDecimalPad;
...
- (CalculatorRow *)fieldRow3{
    if (!_fieldRow3) {
        ...
        _fieldRow3.textField.keyboardType = UIKeyboardTypeNumberPad;
