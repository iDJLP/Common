...
@interface MLineChartView()<UIScrollViewDelegate,UIGestureRecognizerDelegate>
...
- (void)setAxisYImg:(NSString *)imageName{
    _axisView.bgImgView.image=[GColorUtil imageNamed:imageName];
}

- (void)setDateBgColor:(UIColor *)color{
    [_chartView setDateBgColor:color];
}
...
- (void)setIsSingleCharts:(BOOL)isSingleCharts{
    _isSingleCharts = isSingleCharts;
    _chartView.isSingleCharts = isSingleCharts;
    _btnSwichH.hidden = isSingleCharts;
}

- (CFDMLineView*)chartView
{
    if (!_chartView) {
        _chartView = [[CFDMLineView alloc] initWithFrame:CGRectMake(0, CButtonListToMainChartsHight, (SCREEN_WIDTH), ceil(self.height-CButtonListToMainChartsHight)) hasBottomIndex:_hasVol];
        _chartView.hCount = MLineCountDefault;
    }
    return _chartView;
}
